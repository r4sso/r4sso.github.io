            ...
            .then((data) => {
                ...
                if (query !== "" && results.length > 0) {
                    results.forEach((result) => {
                        ...
                        html += `<a href="${result.href}"><div class="mt-4 post-card dark:post-card-dark px-3 py-6 rounded-md shadow-2xl shadow-slate-700/10"><h2 class="text-xl font-semibold">${highlightedTitle}</h2><p class="text-gray-600 dark:text-gray-400 text-sm">${highlightedContent}</p></div></a>`; //    <h3 class="text-end font-medium">${result.categories}</h3>
                    });
                } else if (query !== "") {
